 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    while (c != key.end()) {
      ...
      // 判断当前节点是否有cur指向的字符如果没有则插入，有则进入改节点
    ...
    // 如果当前的key已经存在了，并且是叶子节点那么不能覆盖原来的value,插入失败
    if (end_node != nullptr && end_node->get()->IsEndNode()) {
      return false;
    }
    // 存在但是不是叶子节点,将节点移动构造成叶子节点
    if (end_node != nullptr) {
  ...
  bool Remove(const std::string &key) {
    ...
    while (c != key.end()) {
      auto cur = c++;
      // 如果找到cur所对应的节点就压栈进入下一个字符所在节点
      if (pre_child->get()->HasChild(*cur)) {
        s.push(std::make_tuple(*cur, pre_child));
        pre_child = pre_child->get()->GetChildNode(*cur);
        continue;
      }
      // 没找到cur所对应字符代表key不在Trie树中
      return false;
    }
    ...
    while (!s.empty()) {
      ...
      // 如果这个节点不是叶子节点或者还有其他的key与他有相同的前缀则不能删除
